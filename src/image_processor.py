...
compatible Gemini vision model), making images semantically searchable.
...
def caption_image(
    ...
    """
    Generate a detailed text caption for a single image using Gemini vision.

    Parameters
    ----------
    image_path    : Path to the image file (PNG, JPEG, etc.).
    vision_llm    : An initialised ChatGoogleGenerativeAI instance.
    ...
    try:
        ...
        # Graceful degradation: if vision model is unavailable (quota, model access,
        ...
        print(f"  [image_processor] Vision model unavailable for '{image_path}': {exc}")
...
def process_all_images(
    ...
    for idx, path in enumerate(image_paths, start=1):
        ...
        result = caption_image(path, vision_llm)
